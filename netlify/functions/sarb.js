...
exports.handler = async function(event) {
  ...
  for (const series of SARB_SERIES) {
    try {
      ...
      try {
        ...
        res = await get(url, { rejectUnauthorized: false });
      }

      if (res.status === 200 && res.json) {
        const bond = parseSARB(res.json, series);
        console.log('[sarb] ✓ ' + series + ' → ' + bond.price + '% (' + bond.date + ')');
        return ok({ bond: bond, series: series });
      }
      console.warn('[sarb] ' + series + ' → HTTP ' + res.status + (res.raw ? ' — ' + res.raw.slice(0, 100) : ''));
    } catch (e) {
  ...
  try {
    ...
    if (res.status === 200 && res.json) {
      ...
      return ok({
        ...
        warning: 'SARB API unreachable — using FRED proxy (monthly). Set FRED_API_KEY env var in Netlify for higher rate limits.',
      });
    }
    console.warn('[sarb] FRED → HTTP ' + res.status + (res.raw ? ' — ' + res.raw.slice(0, 100) : ''));
  } catch (e) {
  ...
  return ok({
    ...
    warning: 'Could not reach SARB or FRED. Showing approximate static value. Check Netlify logs.',
  });
};
